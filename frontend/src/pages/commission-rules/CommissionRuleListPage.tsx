...
import type { CommissionRule, ExchangeCorridor, Currency } from '../../types/entities'
...
export default function CommissionRuleListPage() {
  ...
  const fetch = () => {
    setLoading(true)
    commissionRuleRepository.all().then(setData).finally(() => setLoading(false))
  }
  ...
  const openCreate = () => {
    ...
    reset({ exchange_corridor_id: 0, commission_type: 'buy_commission', percent: 0, fixed_amount: 0, fixed_currency_id: null, applies_to: 'origin', is_active: true })
    setModalOpen(true)
  }
  ...
  const openEdit = async (id: number) => {
    ...
    try {
      const [item, cors, curs] = await Promise.all([
        commissionRuleRepository.find(id),
      ...
      reset({
        ...
        commission_type: item.commission_type as any,
        percent: parseFloat(item.percent),
        fixed_amount: parseFloat(item.fixed_amount),
        fixed_currency_id: item.fixed_currency_id,
        applies_to: item.applies_to as any,
  ...
  const onSubmit = async (data: FormData) => {
    try {
      if (isEdit) await commissionRuleRepository.update(editingId!, data)
      else await commissionRuleRepository.create(data)
  ...
  const handleBulkDelete = async (ids: number[]) => {
    ...
    try {
      await Promise.all(ids.map((id) => commissionRuleRepository.delete(id)))
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">{t('nav.commission_rules')}</h1>
      <DataTable<CommissionRule>
        columns={[
          { key: 'commission_type', label: t('commission_rule.type') },
          { key: 'percent', label: '%', render: (item) => `${fmt(item.percent)}%` },
          { key: 'fixed_amount', label: t('commission_rule.fixed_amount'), render: (item) => fmt(item.fixed_amount) },
          { key: 'applies_to', label: t('commission_rule.applies_to') },
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={t(isEdit ? 'commission_rule.edit' : 'commission_rule.create')}>
