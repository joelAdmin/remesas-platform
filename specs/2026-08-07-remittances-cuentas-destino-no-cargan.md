# Spec #1: Select de cuenta destino/origen no respeta la moneda del corredor
...
filtro de moneda del corredor. Además, las cuentas nuevas podían quedar sin `currency_id`, lo que
las hacía desaparecer del select al filtrar.
...
## 3. Comportamiento observado (actual / antes del fix)
- Con corredor seleccionado, el select destino cargaba vacío si no había cuentas en esa moneda (esperado).
- Al crear una cuenta destino desde el `InlineCreateModal`, el `onCreated` ejecutaba
...
- El `InlineCreateModal` no obligaba a asignar moneda, así que la cuenta nueva quedaba con `currency_id`
  null o una moneda distinta, y desaparecía al recargar con el filtro del corredor.

## 4. Comportamiento esperado (después del fix)
...
- Tras crear la cuenta destino/origen, aparece inmediatamente en el select (recarga con el filtro correcto).
...
4. Antes del fix: el select mostraba todas las cuentas del cliente (incluso otras monedas).
5. Crear cuenta destino sin seleccionar país/moneda → la cuenta quedaba sin `currency_id`.
6. Recargar/filtrar por el corredor → la cuenta desaparecía.

## 6. Análisis de causa raíz
### Hipótesis
El `onCreated` del modal recargaba sin el filtro del corredor y el modal no obligaba a asignar moneda.
### Evidencia
...
- `InlineCreateModal.tsx:55-57`: el form inicial solo setea campos con `defaultValue !== undefined`,
  por lo que `currency_id` (select sin default) quedaba vacío y no se enviaba o se enviaba como null.
- `ClientAccountController.php:38-39`: `currency_id` => `nullable` permitía crear cuentas sin moneda.
...
- Crear cuenta desde ClientFormPage sigue permitiendo elegir moneda, pero es obligatorio.
...
| 2026-08-07 | opencode | Aprobada e implementada |